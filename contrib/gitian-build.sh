...
SIGNER=
VERSION=
url=https://github.com/craftcoin/craftcoin
proc=2
mem=2000
...
-u|--url repo       Specify the URL of the repository. Default is https://github.com/craftcoin/craftcoin
...
function move_build_files() {
    find build/out -type f -exec mv '{}' $outputDir/craftcoin-binaries/${VERSION}/ \;
}
...
if [[ $setup == true ]]; then
    git clone https://github.com/craftcoin/gitian.sigs.git
    git clone https://github.com/craftcoin/craftcoin-detached-sigs.git
...
if [[ $build == true ]]; then
    # Make output folder
    mkdir -p $outputDir/craftcoin-binaries/"$VERSION"

    pushd ./gitian-builder || exit 1

    # Clean craftcoin git directory because of old caching
    if [ -d inputs/craftcoin/ ]; then
        echo "Cleaning Dogecoin directory..."
        rm -rf inputs/craftcoin/
    fi
    ...
    for descriptor in "${DESCRIPTORS[@]}"; do
        ...
        echo ""
        ./bin/gbuild -j "$proc" -m "$mem" --commit craftcoin="$COMMIT" --url craftcoin="$url" ../gitian-descriptors/gitian-"$descriptor".yml  || exit 1
