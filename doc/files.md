...
macOS    | `$HOME/Library/Application Support/CraftCoin/`
Windows  | `%APPDATA%\Dogecoin\` <sup>[\[1\]](#note1)</sup>

2. A custom data directory path can be specified with the `-datadir` option.

3. All content of the data directory, except for the `craftcoin.conf` file, is chain-specific. This means the actual data directory paths for non-mainnet cases differ:
...
`./`               | `craftcoin.conf`       | User-defined configuration settings for `craftcoind` or `craftcoin-qt`; can be specified by `-conf` option
`./`               | `craftcoind.pid`        | Stores the process ID (PID) of `craftcoind` or `craftcoin-qt` while running; can be specified by `-pid` option
`./`               | `debug.log`           | Contains debug information and general logging generated by `craftcoind` or `craftcoin-qt`
...
`craftcoin-qt` uses the [`QSettings`](https://doc.qt.io/qt-5/qsettings.html) class; this implies platform-specific [locations where application settings are stored](https://doc.qt.io/qt-5/qsettings.html#locations-where-application-settings-are-stored).
