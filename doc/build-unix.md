...
**Optional dependencies** :
```bash
# Qt (required for craftcoin-qt GUI)
...
According to installed dependencies, the following steps will compile `craftcoind`, `craftcoin-cli` and `craftcoin-qt`.
...
#### Enable Qt GUI
Create `craftcoin-qt`, the core wallet GUI.
```bash
./configure --with-gui
```
