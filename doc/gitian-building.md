...
```bash
gpg --detach-sign ${VERSION}-linux/${SIGNER}/craftcoin-linux-build.assert
gpg --detach-sign ${VERSION}-win/${SIGNER}/craftcoin-win-build.assert
gpg --detach-sign ${VERSION}-osx-unsigned/${SIGNER}/craftcoin-osx-build.assert
```
...
Gitian signatures for each release are added to https://github.com/craftcoin/gitian.sigs.

`gitian-build.sh` will create signatures inside `gitian-output/sigs/` folder. Create a pull request to [craftcoin/gitian.sigs](https://github.com/craftcoin/gitian.sigs) to publish your signatures, the `.assert` and `.assert.sig` files.
